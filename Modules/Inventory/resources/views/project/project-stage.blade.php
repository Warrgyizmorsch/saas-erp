...
══════════════════════════════════════ */
        ...
        .stat-chip.chip-total .dot {
            background: var(--slate);
        }

        .stat-chip.chip-active .dot {
            background: var(--indigo-light);
            box-shadow: 0 0 0 3px rgba(99, 102, 241, 0.15);
        }

        .stat-chip.chip-done .dot {
            background: var(--green-light);
        }

        .stat-chip.chip-locked .dot {
...
══════════════════════════════════════ */
        ...
        .connector-line.done-line {
            background: linear-gradient(180deg, #10b981 0%, #10b98140 100%);
        }

        .connector-line.active-line {
            background: linear-gradient(180deg, #6366f1 0%, #6366f140 100%);
        }

        .connector-line.locked-line {
            background: var(--border);
        }
        ...
        .sc-active .card-stripe {
            background: linear-gradient(90deg, #4f46e5 0%, #a78bfa 60%, transparent 100%);
        }

        .sc-done .card-stripe {
            background: linear-gradient(90deg, #059669 0%, #6ee7b7 60%, transparent 100%);
        }

        .sc-locked .card-stripe {
        ...
        .sc-active .card-stage-tag {
            background: var(--indigo-soft);
            color: var(--indigo);
        }

        .sc-done .card-stage-tag {
            background: var(--green-soft);
            color: var(--green);
        }

        .sc-locked .card-stage-tag {
            background: var(--bg);
            color: var(--text-muted);
        }
        ...
        .sc-active .prog-bar-fill {
            background: linear-gradient(90deg, #4f46e5, #818cf8);
        }

        .sc-done .prog-bar-fill {
            background: linear-gradient(90deg, #059669, #34d399);
        }

        .sc-locked .prog-bar-fill {
            background: var(--border);
        }
...
══════════════════════════════════════ */
        ...
        .si-active .sub-item-title {
            color: #3730a3;
        }

        .si-done .sub-item-title {
            color: var(--text-secondary);
        }

        .si-locked .sub-item-title {
            color: var(--text-muted);
        }
...
══════════════════════════════════════ */
        ...
        /* ══════════════════════════════════════
   PAGE HEADER STRIP — REDESIGNED
══════════════════════════════════════ */
        .pg-header {
            background: linear-gradient(135deg, #f8f9ff 0%, #fff 60%, #f0fdf4 100%);
            border-bottom: 1px solid var(--border);
        ...
        .pg-progress-fill {
            height: 100%;
            background: linear-gradient(90deg, var(--indigo) 0%, #818cf8 50%, var(--green-light) 100%);
            border-radius: 999px;
    ...
    elseif (($st->current_status_name ?? '') == 'in progress') {
    $activeCount++;
    }
    }
    ...
    <div class="pg-shell">
            ...
            <div class="pg-header-inner">
                ...
                <div class="pg-header-body">
                    ...
                    <div class="pg-progress-block">
                        <div class="pg-progress-top">
                            <span class="pg-progress-label">Overall Progress</span>
                            <span class="pg-progress-pct">{{ $overallProgress }}%</span>
                        </div>
                        <div class="pg-progress-track">
                            <div class="pg-progress-fill" style="width: {{ $overallProgress }}%"></div>
                        </div>
                        <div class="pg-progress-hint">
                            {{ $doneCount }} of {{ $totalStages }} stages done
                        </div>
                    </div>
        ...
        <div class="pg-content">
            ...
            <div class="pipeline">
                ...
                $tagLabel = $isDone ? 'Completed' : ($isLocked ? 'Locked' : ($stage->current_status_name ?? 'Pending'));
                    ...
                    <div class="stage-card {{ $cardKey }}">
                        ...
                        <div class="card-head">
                            <div class="card-info">
                                ...
                                <div class="stage-timeline-box mt-3">
                                    ...
                                    <div class="timeline-grid">
                                        ...
                                        <div>
                                            <label>Start</label>
                                            <input type="date"
                                                class="timeline-input timeline-update"
                                                data-type="main"
                                                data-id="{{ $stage->project_main_stage_id ?? $stage->id }}"
                            ...
                            <div class="sub-list">
                                ...
                                <div class="sub-item {{ $siClass }}">
                                    <div class="sub-item-info">
                                        ...
                                        <div class="sub-timeline-box mt-2">

                                            <input type="date"
                                                class="timeline-input timeline-update"
                                                data-type="sub"
                                                data-id="{{ $sub->project_sub_stage_id ?? $sub->id }}"
                                                data-field="start_date"
    ...
    <script>
        ...
        document.addEventListener('change', function(e) {
            ...
            if (e.target.classList.contains('parent-status')) {
                e.preventDefault();
                fetch('/inventory/stage/update-parent-status', {
                        method: 'POST',
                        headers: {
                    ...
                    .then((res) => {
                        if (res.success) {
                            ...
                            Swal.fire({
                                ...
                                position: 'top-end',
                                icon: 'success',
                                title: res.message || 'Stage status updated',
                            ...
                            Swal.fire({
                                icon: 'warning',
                                title: 'Sub stages pending',
                                text: res.message || 'Please complete all sub stages before completing this main stage.',
            ...
            if (e.target.classList.contains('sub-status')) {
                ...
                fetch('/inventory/stage/update-sub-status', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    ...
                    .then((res) => {
                        if (res.success) {
                            ...
                            Swal.fire({
                                ...
                                position: 'top-end',
                                icon: 'success',
                                title: res.message || 'Sub-stage status updated successfully',
                                showConfirmButton: false,
                                timer: 2500,
                                timerProgressBar: true
                            });
        ...
        document.addEventListener('click', function(e) {
            ...
            if (!e.target.classList.contains('save-timeline-btn')) {
                return;
            }

            let btn = e.target;
            let stageId = btn.dataset.id;

            let wrapper = btn.closest('.stage-timeline-box') ||
                btn.closest('.sub-timeline-box');

            let inputs = wrapper.querySelectorAll('.timeline-input');

            let startDate = inputs[0]?.value || '';
            let endDate = inputs[1]?.value || '';

            if (!startDate || !endDate) {
                toast('Please select start and end date', false);
                return;
            }
            ...
            fetch('/inventory/project-stage-timeline/update', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
