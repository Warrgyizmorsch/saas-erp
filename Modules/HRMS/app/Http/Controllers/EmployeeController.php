...
class EmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
                ...
                foreach ($employeeUsers as $u) {
                    ...
                    if (!$emp) {
                        $emp = Employee::create([
                            'name' => $u->name,
                            'email' => $u->email,
                            'mobile_number' => $u->contact_no ?? '0000000000',
                            'employee_code' => 'EMP-' . str_pad($u->id, 4, '0', STR_PAD_LEFT),
                            'role' => 'employee',
                            'working_mode' => 'Office',
                            'department' => 'Web Development', // Default
        ...
        return view('hrms::employees.index', compact(
            'employees',
            'perPage',
    ...
    public function store(Request $request)
    {
        try {
            ...
            return DB::transaction(function () use ($request) {
                ...
                // Create User for login if email is present and limit not reached
                $userCreated = false;
                ...
                if ($request->filled('email')) {
                    ...
                    if ($maxUsers === null || $currentUsers < (int)$maxUsers) {
                        User::create([
                            ...
                            'role_id' => 2, // Always assign 'Employee' (ID 2) for employee logins
                ...
                if ($request->filled('email') && !$userCreated) {
                    return redirect()->route('employees.index')
                        ->with('success', "Employee added successfully! (User login account could not be created because the subscription plan's limit of {$maxUsers} users has been reached.) ✓");
                }
    ...
    //     $roleSlug = auth()->user()->hrm_role; // e.g. "manager"
    ...
    //     $attendances = \Modules\HRMS\App\Models\Attendance::where('employee_id', $id)
    //         ->whereBetween('attendance_date', [$startDate, $endDate])
    //         ->get()
    //         ->keyBy(function($item) { return $item->attendance_date->format('Y-m-d'); });

    //     $holidays = \Modules\HRMS\App\Models\Holiday::whereBetween('date', [$startDate, $endDate])
    //         ->get()
    //         ->keyBy(function($item) { return \Carbon\Carbon::parse($item->date)->format('Y-m-d'); });

    //     $leaves = \Modules\HRMS\App\Models\LeaveApplication::where('employee_id', $id)
    ...
    public function getAttendance(Request $request, $id = null)
    {
        ...
        // Fetch data
        $attendances = \Modules\HRMS\App\Models\Attendance::where('employee_id', $employeeId)
            ->whereBetween('attendance_date', [$startDate, $endDate])
            ->get()
        ...
        $leaves = \Modules\HRMS\App\Models\LeaveApplication::where('employee_id', $employeeId)
        ...
        return view('hrms::payroll.attendance-history', [
            'employee' => $employee,
    ...
    public function employeeDays() {
        ...
        return view('hrms::employees.employeeDay', compact('employees'));
    }
        ...
        try {
            ...
            return DB::transaction(function () use ($request, $employee, $user) {
                ...
                if ($request->filled('email')) {
                    ...
                    $role = Role::all()->first(function($r) use ($request) {
                        return \Illuminate\Support\Str::slug($r->name, '_') === $request->role;
                    });
                    ...
                    if (!$userSynced) {
                        return redirect()->route('employees.index')
                            ->with('success', "Employee updated successfully! (User login account could not be created because the subscription plan's limit of {$maxUsers} users has been reached.) ✓");
                    }
                }
