...
class EmployeeDashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        // =========================
        if ($auth_rol_id == '5' || $auth_rol_id == '6') {
            ...
            $data['rs'] = RequestSlip::where('employee_id', $auth_id)->whereBetween('created_on', [$from, $to])->count();
            $data['rejected_rs'] = RequestSlip::where('employee_id', $auth_id)->where('status', 'Rejected')->whereBetween('created_on', [$from, $to])->count();
            $data['hold_rs'] = RequestSlip::where('employee_id', $auth_id)->where('status', 'Hold')->whereBetween('created_on', [$from, $to])->count();
            $data['pending_rs'] = RequestSlip::where('employee_id', $auth_id)->where('status', 'Pending')->whereBetween('created_on', [$from, $to])->count();
            $data['approved_rs'] = RequestSlip::where('employee_id', $auth_id)->where('status', 'Approved')->whereBetween('created_on', [$from, $to])->count();
            $data['exceed_rs'] = RequestSlip::where('employee_id', $auth_id)->where('is_exited', '1')->whereBetween('created_on', [$from, $to])->count();
        }
        ...
        if ($auth_rol_id == '5') {
            ...
            $data['issue'] = Issue::where('employee_id', $auth_id)->whereBetween('created_on', [$from, $to])->count();

            $data['completed_issued'] = Issue::where('employee_id', $auth_id)->where('status', 'Issued')
                ->whereBetween('created_on', [$from, $to])
                ->count();
            $data['partially_issue'] = Issue::where('employee_id', $auth_id)->where('status', 'Partially Issued')
                ->whereBetween('created_on', [$from, $to])
                ->count();
        }
        ...
        if ($auth_rol_id == '5') {
            ...
            $data['total_job_cart'] = JobCard::where('employee_id', $auth_id)->whereBetween('created_at', [$from, $to])->count();

            $data['completed_job_cart'] = JobCard::where('employee_id', $auth_id)->where('status', 'COMPLETED')
                ->whereBetween('created_at', [$from, $to])
                ->count();

            $data['Pending_job_card'] = JobCard::where('employee_id', $auth_id)->where('status', 'PENDING')
                ->whereBetween('created_at', [$from, $to])
                ->count();
        }
        ...
        if ($auth_rol_id == '5') {
            ...
            $data['total_po'] = PurchaseOrder::where('created_by', $auth_id)->whereBetween('created_at', [$from, $to])->count();

            $data['completed_po'] = PurchaseOrder::where('created_by', $auth_id)->where('status', 'Completed')
                ->whereBetween('created_at', [$from, $to])
                ->count();

            $data['completed_draft'] = PurchaseOrder::where('created_by', $auth_id)->where('status', 'Draft')
                ->whereBetween('created_at', [$from, $to])
                ->count();

            $data['completed_apv'] = PurchaseOrder::where('created_by', $auth_id)->where('status', 'Approved')
                ->whereBetween('created_at', [$from, $to])
                ->count();

            $data['completed_submitted'] = PurchaseOrder::where('created_by', $auth_id)->where('status', 'Submitted')
                ->whereBetween('created_at', [$from, $to])
                ->count();

            $data['po_pr'] = PurchaseOrder::where('created_by', $auth_id)->where('status', 'Partially Received')
                ->whereBetween('created_at', [$from, $to])
                ->count();
        }
        ...
        //Purchase Request
        if ($auth_rol_id == '4') {
            $data['total_pr'] = PurchaseRequest::whereBetween('created_at', [$from, $to])->count();

            $data['purchase_submitted'] = PurchaseRequest::where('status', 'SUBMITTED')
                ->whereBetween('created_at', [$from, $to])
                ->count();

            $data['purchase_apv'] = PurchaseRequest::where('requested_by', $auth_id)->where('status', 'APPROVED')
                ->whereBetween('created_at', [$from, $to])
                ->count();
        } else {
            $data['total_pr'] = PurchaseRequest::where('requested_by', $auth_id)->whereBetween('created_at', [$from, $to])->count();
            $data['purchase_draft'] = PurchaseRequest::where('requested_by', $auth_id)->where('status', 'DRAFT')
                ->whereBetween('created_at', [$from, $to])
                ->count();
            $data['purchase_submitted'] = PurchaseRequest::where('requested_by', $auth_id)->where('status', 'SUBMITTED')
                ->whereBetween('created_at', [$from, $to])
                ->count();
            ...
            $data['purchase_rejected'] = PurchaseRequest::where('requested_by', $auth_id)->where('status', 'REJECTED')
                ->whereBetween('created_at', [$from, $to])
                ->count();

            $data['purchase_hold'] = PurchaseRequest::where('requested_by', $auth_id)->where('status', 'HOLD')
                ->whereBetween('created_at', [$from, $to])
                ->count();

            $data['purchase_par_order'] = PurchaseRequest::where('requested_by', $auth_id)->where('status', 'PARTIALLY_ORDERED')
                ->whereBetween('created_at', [$from, $to])
                ->count();

            $data['purchase_order'] = PurchaseRequest::where('requested_by', $auth_id)->where('status', 'ORDERED')
                ->whereBetween('created_at', [$from, $to])
                ->count();

            $data['purchase_cls'] = PurchaseRequest::where('requested_by', $auth_id)->where('status', 'CLOSED')
                ->whereBetween('created_at', [$from, $to])
                ->count();
        }
        // Grn
        $data['total_grn'] = Grn::whereBetween('created_at', [$from, $to])->count();
        ...
        //recent pending grn
        $data['recent_pending_grn'] = PurchaseOrder::with('creator')->where('status', 'Approved')
            ->whereBetween('created_at', [$from, $to])
        ...
        $data['recent_pending_po'] = PurchaseOrder::with('creator')
            ...
            ->get();
        if ($auth_rol_id == '5') {
            $data['recent_pending_rs'] = RequestSlip::with('creator')
            ->whereIn('status', ['Pending', 'Approved HOD'])
            ->whereNotNull('created_by')
            ->whereBetween('created_on', [$from, $to])
            ->orderBy('created_on', 'desc')
            ->take(5)
        ...
        $data['recent_rs'] = RequestSlip::where('employee_id', $auth_id)->with(['project', 'creator'])
            ->whereBetween('created_on', [$from, $to])
            ->orderBy('created_on', 'desc')
            ->take(5)
            ->get();

        $data['recent_issues'] = Issue::where('employee_id', $auth_id)->with(['project', 'creator'])
            ->whereBetween('created_on', [$from, $to])
            ->orderBy('created_on', 'desc')
            ->take(5)
            ->get();

        $data['recent_job_carts'] = JobCard::where('employee_id', $auth_id)
            ->whereBetween('created_at', [$from, $to])
            ->orderBy('created_at', 'desc')
            ->take(5)
            ->get();
        ...
        $project_ids = RequestSlip::where('employee_id', $auth_id)
            ->pluck('project_id');
        ...
        $last6Start = Carbon::now()->subMonths(5)->startOfMonth()->startOfDay();
        ...
        switch ($auth_rol_id) {
            ...
            case '2':
            case '5':
                return view('inventory::storeDashboard', $data);

            case '9':
                return view('inventory::salesDashboard', $data);

            case '7':
                return view('inventory::employeDashboard', $data);

            case '6':
                return view('inventory::hodDashboard', $data);

            case '4':
            case '10':
                return view('inventory::purchaseDashboard', $data);
