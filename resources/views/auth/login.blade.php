...
<head>
    ...
    <title>Log In | Enterprise SaaS ERP</title>
...
<body>
    <div class="auth-card">
        <div class="text-center">
            ...
            <p class="auth-subtitle">Log in to manage your professional workspace</p>
        </div>

        <!-- Session Status / Errors -->
        @if (session('status'))
            <div class="alert alert-success border-0 rounded-3 shadow-sm mb-4" role="alert" style="font-size: 13px;">
                {{ session('status') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('login') }}" class="needs-validation" novalidate>
            ...
            <div class="mb-4">
                ...
                <div class="position-relative">
                    <input id="email" type="email" name="email" class="form-control form-control-custom" 
                           placeholder="name@company.com" value="{{ old('email') }}" required autofocus autocomplete="username">
                </div>
            </div>
            ...
            <div class="mb-4">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    ...
                    @if (Route::has('password.request'))
                        <a class="link-custom fs-12" href="{{ route('password.request') }}" style="font-size: 12px;">
                            Forgot password?
                        </a>
                    @endif
                </div>
            ...
            <div class="mb-4 form-check d-flex align-items-center gap-2">
                <input id="remember_me" type="checkbox" class="form-check-input form-check-input-custom" name="remember">
                <label class="form-check-label text-muted fs-14" for="remember_me" style="font-size: 14px; user-select: none;">Remember this device</label>
            </div>
            ...
            <div class="text-center text-muted fs-14 mt-4" style="font-size: 14px;">
                ...
                <a href="{{ route('register') }}" class="link-custom">Create one</a>
            </div>
        </form>
    </div>
