            ...
            <div class="card-body">
                <form id="servicePageForm" action="{{ route('warr-service-pages.store') }}" method="POST">
                        ...
                        <div class="col-lg-9">
                            <select name="service_id" class="form-select" required>
                                ...
                                @foreach($services as $s)
                                    <option value="{{ $s->id }}" {{ (string) old('service_id', $clone->service_id ?? '') === (string) $s->id ? 'selected' : '' }}>
                                        {{ $s->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-lg-9">
                            <select name="country_id" id="countrySelect" class="form-select" required>
                                ...
                                @foreach($countries as $c)
                                    <option value="{{ $c->id }}" {{ (string) old('country_id', $clone->country_id ?? '') === (string) $c->id ? 'selected' : '' }}>
                                        {{ $c->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="city_id" id="citySelect" class="form-select">
                                ...
                                @if(!empty($cities))
                                    @foreach($cities as $ct)
                                        <option value="{{ $ct->id }}" {{ (string) old('city_id', $clone->city_id ?? '') === (string) $ct->id ? 'selected' : '' }}>
                                            {{ $ct->name }}
                                        </option>
                                    @endforeach
                                @endif
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-lg-9">
                            <select name="more_services[]" id="moreServicesSelect" class="form-select" multiple
                                data-select2-selector="tag" data-placeholder="Select services (leave empty for all)"
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-lg-9">
                            <textarea name="hero_description" class="form-control"
                                rows="4">{{ old('hero_description', $clone->hero_description ?? '') }}</textarea>
                        </div>
                    </div>
                    ...
                    @for($i = 1; $i <= 4; $i++)
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-lg-9">
                                <textarea name="section{{ $i }}_description" class="form-control"
                                    rows="4">{{ old("section{$i}_description", data_get($clone ?? null, "section{$i}_description", '')) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            ...
                            <button type="button" class="btn btn-success btn-sm add-point" data-section="{{ $i }}">+ Add
                                Point</button>
                        </div>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-lg-9">
                            <textarea name="meta_description" class="form-control"
                                rows="3">{{ old('meta_description', $clone->meta_description ?? '') }}</textarea>
                        </div>
                    </div>
